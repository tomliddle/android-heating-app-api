...
object HeatingActor {
	...
	case class HeatingStatusAll(status: Status, targetTemp: Option[BigDecimal], currentTemp: Option[BigDecimal]) {
		def toResult(w: WeatherStatus): Result = Result(status = status.toString, targetTemp = targetTemp, currentTemp = currentTemp, outsideTemp = w.outsideTemp, outlook = w.outlook)
	}

	case class Result(status: String, targetTemp: Option[BigDecimal], currentTemp: Option[BigDecimal], outsideTemp: Option[BigDecimal], outlook: Option[String])
...
/**
	...
	*/
class HeatingActor(environment: Environment, configuration: Configuration) extends Actor with ActorLogging {
	...
	//logger.error(configuration.underlying.getString("app.bindir"))
	private lazy val binPath = s"${environment.rootPath.getAbsolutePath}/conf/bin-test"
	...
	private def setBurnerOn() {
		burnerOn = Some(true)
		callCommand(s"heating_on")
	}

	private def setBurnerOff() {
		burnerOn = Some(false)
		callCommand(s"heating_off")
	}
